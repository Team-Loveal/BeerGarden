  ...
  Widget build(BuildContext context) {
    ...
    Firestore.instance
        ...
        .listen((data) => data.documents.forEach((doc) => {
              //check if matchForToday quota is true
              if (doc['matchForToday'])
                {
                  {matchID = doc['toID'], chatID = doc.documentID}
                }
              //if user hasn't been matched to a new user today yet
              // find a user that hasn't been matched before
              else
                {
                  if (!doc['matched'])
                    {matchID = doc['toID'], chatID = doc.documentID}
    ...
    Firestore.instance
        .collection("messages")
        .document(chatID)
        .updateData({'matched': true, 'matchForToday': true});
